...
pub struct ResolveDispute<'info> {
    #[account(address = task_resolution.arbitration_authority @ ErrorCode::Unauthorized)]
...
pub fn handle_resolve_dispute(ctx: Context<ResolveDispute>, outcome: DisputeOutcome) -> Result<()> {
    ...
    ctx.accounts.task_resolution.resolve(outcome, timestamp)?;
    settle_dispute(
    ...
    emit!(DisputeResolved {
        ...
        actor: ctx.accounts.arbitration_authority.key(),
